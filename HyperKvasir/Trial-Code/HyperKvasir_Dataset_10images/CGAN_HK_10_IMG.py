...
class ImageDataset(Dataset):
    def __init__(self, root, transform=None, mode='train'):
        ...
        self.new_perm()
        assert len(self.files_A) > 0, "Make sure you downloaded the HyperKvasir_Dataset_10 images!"
...
import torchvision
dataset = ImageDataset("HyperKvasir_Dataset_10images", transform=transform)
...
def train(save_model=False):
    ...
    for epoch in range(n_epochs):
        for real_A, real_B in tqdm(dataloader):
            ...
            # Keep track of losses and save to CSV
            ...
            if cur_step % display_step == 0:
                ...
                if save_model:
                    ...
                    }, f"cycleGAN_HK_{cur_step}.pth")
            cur_step += 1

            #loss_file = open("losses.csv", "w", newline="")  # Open in write mode
            #loss_file.close()  # Close the CSV file

            with open("losses.csv", "a", newline="") as loss_file:
                csv_writer = csv.writer(loss_file)
                csv_writer.writerow([epoch, cur_step, mean_generator_loss, mean_discriminator_loss])
            ...
            for i in range(len(fake_B)):
                image_fake = fake_B[i].cpu().detach().numpy()
                image_fake_rgb = np.clip(image_fake * 255, 0, 255).astype(np.uint8)  # Assuming pixel values are in [0, 1] range
                image_fake_rgb = np.transpose(image_fake_rgb, (1, 2, 0))  # Change channel order if necessary
                plt.imsave(os.path.join(output_dir, f"fake_B_{cur_step}_{i}.png"), image_fake_rgb)

                image_real = real_B[i].cpu().detach().numpy()  # Assuming real_B contains real images
                image_real_rgb = np.clip(image_real * 255, 0, 255).astype(np.uint8)
                image_real_rgb = np.transpose(image_real_rgb, (1, 2, 0))
                plt.imsave(os.path.join(output_dir, f"real_B_{cur_step}_{i}.png"), image_real_rgb)

            for i in range(len(fake_A)):
                image_fake = fake_A[i].cpu().detach().numpy()
                image_fake_rgb = np.clip(image_fake * 255, 0, 255).astype(np.uint8)
                image_fake_rgb = np.transpose(image_fake_rgb, (1, 2, 0))
                plt.imsave(os.path.join(output_dir, f"fake_A_{cur_step}_{i}.png"), image_fake_rgb)

                image_real = real_A[i].cpu().detach().numpy()  # Assuming real_A contains real images
                image_real_rgb = np.clip(image_real * 255, 0, 255).astype(np.uint8)
                image_real_rgb = np.transpose(image_real_rgb, (1, 2, 0))
                plt.imsave(os.path.join(output_dir, f"real_A_{cur_step}_{i}.png"), image_real_rgb)
